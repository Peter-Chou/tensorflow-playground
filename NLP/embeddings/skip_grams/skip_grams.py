...
class SkipGrams(object):
    ...
    def __init__(self, text, n_counts_to_keep, subsample_threshold=1e-5,
                 window_size=5, epochs, batch_size, n_embedding, n_sampled):
        self.text = text
...
def main():
    ...
    model = SkipGrams(text, N_COUNTS_TO_KEEP, epochs=EPOCHS, batch_size=BATCH_SIZE,
                      n_embedding=N_EMBEDDING, n_sampled=N_SAMPLED)
    model.build()
    model.train()
